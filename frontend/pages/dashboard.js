...
export default function Dashboard() {
  ...
  const loadBalances = async (userAccount) => {
    ...
    try {
      ...
      // Load contract balance using new AI Yield Aggregator
      const contract = new ethers.Contract(CONTRACT_ADDRESS, CONTRACT_ABI, provider);
      const userPosition = await contract.getUserPosition(userAccount);
      const [totalDeposited, totalWithdrawn, currentYield, totalFees, isActive] = userPosition;
      setContractBalance(parseFloat(ethers.formatUnits(totalDeposited - totalWithdrawn, 6)));
  ...
  const depositUSDC = async () => {
    ...
    try {
      ...
      // Deposit to AI Yield Aggregator (automatically farms yield)
      ...
      alert(`🚀 Successfully deposited ${depositAmount} USDC! The AI is now automatically farming yield across DeFi protocols!`);
  ...
  const withdrawUSDC = async () => {
    ...
    try {
      ...
      // Withdraw all available balance (includes yield and fees)
      const withdrawTx = await contract.withdraw(ethers.parseUnits(contractBalance.toString(), 6));
      await withdrawTx.wait();
      
      // Reload balances
      await loadBalances(account);
      
      alert(`💰 Successfully withdrew ${contractBalance.toFixed(2)} USDC! This includes your original deposit plus any yield generated by the AI!`);
